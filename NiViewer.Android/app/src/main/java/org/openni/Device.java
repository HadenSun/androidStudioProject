...
public class Device {
	...
	{
		OutArg<FWVersion> val = new OutArg<FWVersion>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetAXonLinkFWVersionProperty(getHandle(), NativeMethods.DEVICE_PROPERTY_FIRMWARE_VERSION, val));
		return val.mValue;
	}
	public final Version getDriverVersion()
	{
		OutArg<Version> val = new OutArg<Version>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetDriverVersion(getHandle(),NativeMethods.DEVICE_PROPERTY_DRIVER_VERSION,val));
		return val.mValue;
	}
	public final CamParam getCamParam()
	{
		OutArg<CamParam> val = new OutArg<CamParam>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetAXonLinkCamParamProperty(getHandle(), NativeMethods.AXONLINK_DEVICE_PROPERTY_GET_CAMERA_PARAMETERS, val));
		return val.mValue;
	}
	public final String getDeviceSerialNumber()
	{
		OutArg<String> val = new OutArg<String>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetAXonLinkSerialNumberProperty(getHandle(), NativeMethods.DEVICE_PROPERTY_SERIAL_NUMBER, val));
		return val.mValue;
	}
	public final UpgradeStatus getUpgradeStatus()
	{
		OutArg<Integer> val = new OutArg<Integer>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetIntInvoke(getHandle(), NativeMethods.AXONLINK_DEVICE_INVOKE_GET_UPGRADE_STATUS,val));
		return UpgradeStatus.fromNative(val.mValue);
		
	}
	public final void SetUpgradeFile(String filepath)
	{
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceUpgradeFileInvoke(getHandle(),filepath));
	}
	public final void UpgradeEnable(boolean enable)
	{
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceSetBoolInvoke(getHandle(), NativeMethods.AXONLINK_DEVICE_INVOKE_SET_UPGRADE_ENABLE, enable));
	}
	public final void reboot()
	{
		NativeMethods.oniDeviceSetBoolInvoke(getHandle(),NativeMethods.AXONLINK_DEVICE_INVOKE_SET_REBOOT,true);
	}
	public final FirmWarePacketVersion getFirmWarePacketVersion(String filename) 
	{
		OutArg<FirmWarePacketVersion> val = new OutArg<FirmWarePacketVersion>();
		NativeMethods.checkReturnStatus(NativeMethods.oniDeviceGetFirmWarePacketVersion(getHandle(),filename,val));
		return val.mValue;
	}
