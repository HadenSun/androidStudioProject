...
*/
    ...
    public void onConfigurationChanged(Configuration config) {
        ...
        if (Configuration.ORIENTATION_PORTRAIT == config.orientation) {
            mStreamsContainer.setOrientation(LinearLayout.VERTICAL);//指定控件的排列方向
        } else {
        ...
        for (StreamView streamView : getStreamViews()) {//得到视图
            mStreamsContainer.removeView(streamView);
            setStreamViewLayout(streamView, config);//调整视图 设置视图的具体大小形状等
            mStreamsContainer.addView(streamView);//动态添加视图
        }

        super.onConfigurationChanged(config);//屏幕横竖屏
    }
    ...
    private void  addStream(int pos) {
        StreamView streamView = new StreamView(this);
        setStreamViewLayout(streamView, getResources().getConfiguration());


        streamView.setDevice(mDevice,pos);
        mStreamsContainer.addView(streamView);
        mStreamsContainer.requestLayout();
    }
    ...
    private List<StreamView> getStreamViews() {
        int count = mStreamsContainer.getChildCount();//视图的数量

        ArrayList<StreamView> list = new ArrayList<StreamView>(count);
        for (int i = 0; i < count; ++i) {
            StreamView view = (StreamView)mStreamsContainer.getChildAt(i);//返回指定位置的视图
            list.add(view);//增加元素
        }
        return list;
    }
